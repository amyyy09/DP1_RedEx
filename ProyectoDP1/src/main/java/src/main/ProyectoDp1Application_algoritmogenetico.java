...
public class ProyectoDp1Application_algoritmogenetico {
    ...
    for (int i = 0; i < numGeneraciones; i++) {
        ...
        if (!fitnessAgregado.isEmpty() && fitnessAgregado.get(0) >= 0) {
            // Revisar si el cromosoma es válido -> crear función en relación al almacén
        ...
        for (int j = 0; j < numDescendientes / 2; j++) {
            ...
            if (Math.random() < Pc) {
                ...
                if (Math.random() < Pm) {
                    // Aquí aplica la mutación a los hijos
                    mutarHijos(hijos, rutas); // Suponiendo que existe este método
                }
...
// Considera implementar aquí los métodos calcularFitnessAgregado, tournamentSelection, crossover, mutarHijos, etc.
	public List <Cromosoma> mutarHijos(List<Cromosoma> hijos, List<Ruta> rutas){
