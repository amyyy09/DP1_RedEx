...
public class FitnessEvaluatorService {
    ...
    public double fitness(Map<Paquete, RutaTiempoReal> particula, List<Aeropuerto> aeropuertos,
        ...
        for (Entry<Paquete, RutaTiempoReal> entrada : particula.entrySet()) {
            ...
            for (int i = 0; i < vuelos.size(); i++) {
                ...
                usoCapacidadVuelos.put(vuelos.get(i).getIdVuelo(),
                        usoCapacidadVuelos.getOrDefault(vuelos.get(i).getIdVuelo(), 0) + 1);
                ...
                Almacen almacenOrigen = aeropuertos.stream().filter(a -> a.getCodigoIATA().equals(codigoIATAOrigen))
                        .findFirst().get().getAlmacen();
                // int capacidadOrigen = almacenOrigen.getCapacidad();
                int ocupacionOrigen;
                // verificar que en el map de usoCapacidadAlmacenesOrigen no exista fechas
                // anteriores a la fecha actual, es un treemap ordenado por fecha
                ...
                NavigableMap<LocalDateTime, Integer> tailMap = usoCapacidadAlmacenesOrigen
                        .tailMap(vuelos.get(i).getHoraSalida(), false);

                for (Map.Entry<LocalDateTime, Integer> entry : tailMap.entrySet()) {
                    usoCapacidadAlmacenesOrigen.put(entry.getKey(), entry.getValue() - 1);
                }

                Almacen almacenDestino = aeropuertos.stream().filter(a -> a.getCodigoIATA().equals(codigoIATADestino))
                        .findFirst().get().getAlmacen();
                ...
                // verificar que en el map de usoCapacidadAlmacenesDestino no exista fechas
                // anteriores a la fecha actual, es un treemap ordenado por fecha
                ...
                NavigableMap<LocalDateTime, Integer> tailMapDestino = usoCapacidadAlmacenesDestino
                        .tailMap(vuelos.get(i).getHoraLlegada(), false);
        ...
        for (Map.Entry<String, TreeMap<LocalDateTime, Integer>> entry : usoCapacidadAlmacenes.entrySet()) {
            ...
            Almacen almacen = aeropuertos.stream().filter(a -> a.getCodigoIATA().equals(key)).findFirst().get()
                    .getAlmacen();
    ...
    public Vuelo encontrarVueloActual(List<Vuelo> vuelosActivos, RutaTiempoReal ruta) {
    ...
    public Almacen encontrarAlmacenActual(List<Aeropuerto> aeropuertos, String codigoIATA) {
